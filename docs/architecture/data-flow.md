...
| 禁止模式 | 说明 | 违规原因 |
|---------|------|---------|
| OutputHandler 订阅 Input 事件 | `data.message` | 绕过 Decision 阶段，破坏分层架构 |
| Decider 订阅 Output 事件 | `render.completed` 等 | 创建循环依赖，破坏单向流 |
| InputCollector 订阅 Decision/Output 事件 | `decision.intent` 等 | Input 应只发布，不订阅下游 |

### 详细说明

#### 为什么禁止 OutputHandler 订阅 Input 事件？

OutputHandler 如果直接订阅 `data.message` 事件，会绕过 Decision 阶段，导致：
- 未经决策的原始数据直接输出
- 破坏了 3 阶段架构的分层原则
- 无法保证输出的可控性和安全性
...
| 事件名 | 发布者 | 订阅者 | 数据类型 | 说明 |
|--------|--------|--------|---------|------|
| `data.message` | InputCollector | DeciderManager | `NormalizedMessage` | 标准化消息就绪 |
| `decision.intent` | Decider | OutputHandlerManager | `Intent` | 决策意图生成 |
| `output.intent` | ExpressionGenerator | OutputHandlers | `RenderParameters` | 过滤后的渲染参数 |
| `render.completed` | OutputHandlers | - | `RenderCompletedPayload` | 渲染完成通知 |
...
class MyDecider(Decider):
    ...
    async def _handle_message(self, event_name: str, data: MessageReadyPayload, source: str):
        ...
        await self._event_bus.emit(
            CoreEvents.DECISION_INTENT,
            IntentPayload.from_intent(intent, provider=self.decider_name),
