...
| 人物画像 | `viewer_profiles` 画像文本（本批发言人、有画像才注入、人数上限可配） |

组装顺序固定：环节描述 → 直播间快照 → 人物画像。**空段规则：有数据才渲染段**（无占位文本）。参考段只追加在尾部、不在中间插入——后续 ReAct 循环的 assistant/tool 消息继续 append-only 追加其后，保持整个消息序列的前缀稳定。
