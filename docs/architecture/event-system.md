...
| `live.started` | `LiveStartedPayload` | `LiveSessionManager`（`open_session`，手动开启 / 模拟器回放自动开启） | `EventRecorder`、`Broadcaster`（WS type `live.started`）、观察器 | 显式场次开启；Payload 含 `live_session_id`（INTEGER 主键）/ `source`（manual/replay）/ `title` / `room_id` / `started_at_ms`。默认场次（scratch 兜底行）复用**不**发本事件 |
