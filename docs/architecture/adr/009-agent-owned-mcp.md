...
maicraft MCP server 是 MinecraftAgent 的实现内部工具（游戏内操作能力），但配置位于全局
...
2. **通用工具列表被内部工具污染**：maicraft_* 工具进入所有消费方（主播 Planner 等）的
   一般工具列表——其他 Agent 既不需要也不该看见它们，只留下幻觉编名调用的风险面。
3. **通用通道承载特定内部工具**：`[tools.mcp]` 的语义本应是"全局可见的外部工具源"，
   特定 Agent 的实现内部工具混入其中，通用与私有失去边界。
...
2. **显式域查询** `list_tools(provider="maicraft")`：域内查询不受归属过滤——
   MinecraftAgent 的动态工具列表发现零改动
3. **运营查询** `list_tools(include_scoped=True)`：Dashboard 工具页显式看见一切，
   条目携带 `owner_agent` 字段

### 4. invoke 不校验归属

受众治理只管可见性。LLM 幻觉编名直调保留工具是**已知边界**（与手动停用同级别的
...
**拒绝**。每个工具列表消费方各自记得过滤是脆弱约定，新增消费方即踩坑——与思考流
