...
| 升什么位 | 判定 |
|---|---|
| PATCH `2.0.x` | 已有功能的修复、完善与调通——包括架构代际迁移后逐个调通存量功能；为此新增的采集器/工具/事件族等构件随整体算修复 |
| MINOR `2.x.0` | 新增成块的大功能：一个新功能域（如 Rundown 流程单、弹幕模拟器）或一个新 Agent，配置向后兼容 |
| MAJOR `x.0.0` | 架构代际更替（如插件架构 → 三阶段流水线架构 → Agent 架构）、配置/存储需用户手动干预的不兼容升级 |

纪律：

- **按变更整体的意图定性，不按单笔形式机械套表**：为修复或完善某个已有功能而新增的采集器、工具、事件族等构件（例：为修复 Minecraft Agent 身体感知链而新增的注意流采集器与 `game.body.*` 事件族），随整体归入修复，升 PATCH；MINOR 的"新增"指面向用户开放了此前不存在的能力。
