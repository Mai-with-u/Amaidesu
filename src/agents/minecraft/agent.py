...
_GAMEPLAY_RULES = (
    ...
    # 机器正常路径只让模型理解工艺、勘测场地和设计；供料、安装与施工核验交给 Mod。
    "机器建造按四步推进：读取 MCP Resource 中的目标工艺；perceive(view=construction_site) 勘测场地；"
    "据此生成显式 blueprint 与 assembly；plan(goal.ability=maicraft:build_machine) 校验后，"
    "execute(plan_id=返回编号) 交给 Mod 建造。design_machine 是可选审阅，不是必经前置步骤。"
    "由你选择设备、工件承载面、输送回流和动力方案；复用已读蓝图格式与部件资料，"
    "仅为当前方案的具体缺口补读 maicraft://knowledge/machine_assembly 或选中部件的原生接口。"
    "资源中的实际产物与工艺足以解释用户口语名称时，记录采用的解释并设计，不要求目录存在同名整机。"
    ...
    "plan 或施工返回具体问题时按 design_diagnostics 或 validation.issues 修订原蓝图，保留目标与禁用模组；"
