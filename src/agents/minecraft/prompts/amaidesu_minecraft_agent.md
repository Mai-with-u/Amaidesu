...
- **minecraft_todo**（read / write）：你的待办文档。分配给你的指令需要你**自己**分解为多条 todo 并逐项推进到 `done`。复用最近读写返回的全文；内容缺失时才 `read`，改完用 `write` 覆盖（全量文档，无 id）。
...
- 查询应解决当前动作的具体信息缺口。根据已有工艺和现场形成方案，交给 `plan` 校验，再按返回的具体问题补查或修订。库存、供电和运行验收属于后续执行条件，不要求先证明这些条件全部成立才能设计；已知不可实现的部件或连接应修改方案。
