...
- **minecraft_observation**：索引和读取本任务已有观察。回执正文已完整提供；历史整理后缺少具体原文时，用 `ref`、`path` 定位补读。当前任务事实中的决策编号、选项和失败证据可直接用于应答；历史观察不证明当前世界未变。
...
- 本次查询若标记 `same_request_and_result=true`，表示又取得了相同内容。复用已有证据推进；确需继续查询时针对缺少的字段、不同资料或已变化的现场，避免反复读取整个目录。
