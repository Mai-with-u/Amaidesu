...
- 查询应解决一个明确的信息缺口。已有可用产物、已知执行入口且没有未解决的前置条件时，沿原目标推进下一待办；子任务完成不代表整个任务完成，局部的“只审阅”也不能覆盖玩家最终要求建好的目标。
...
- 同一任务跨等待保留历史，正常轮次只追加消息。达到预算时集中整理一次，原始要求、待办与任务事实独立保留；历史摘要不能改变玩家授权或把未知结果说成成功。
