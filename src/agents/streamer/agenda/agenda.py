...
__all__ = ["AgendaBranch", "AgendaSegment", "Agenda", "parse_agenda_toml"]
...
class AgendaBranch(BaseModel):
    """节目单环节内的可选分支,描述 AI 触发的跳转条件与目标。"""

    model_config = ConfigDict(extra="forbid")

    branch_id: str = Field(..., description="分支唯一标识(在同一 AgendaSegment 内唯一)")
...
class AgendaSegment(BaseModel):
    """直播节目单中单个环节——一段有目标 / 时长 / 任务描述的直播片段。"""

    model_config = ConfigDict(extra="forbid")

    id: str = Field(..., description="环节唯一标识(用于分支跳转定位,节目单内唯一)")
    ...
    branches: List[AgendaBranch] = Field(
        default_factory=list,
...
class Agenda(BaseModel):
    """整场直播节目单——若干环节的集合,带可选回退目标。"""

    model_config = ConfigDict(extra="forbid")

    agenda_id: str = Field(..., description="节目单唯一标识")
    title: str = Field(..., description="节目单标题,面向人展示")
    segments: List[AgendaSegment] = Field(
        ...,
        min_length=1,
        description="环节列表,非空;每个环节 id 在节目单内唯一",
    )
    ...
    def _validate_integrity(self) -> "Agenda":
        ...
        if not self.segments:
            raise ValueError("Agenda.segments 不能为空,至少需要一个环节")
        ...
        if len(seg_ids) != len(self.segments):
            ...
            for seg in self.segments:
                if seg.id in seen and seg.id not in dupes:
                    dupes.append(seg.id)
                seen.add(seg.id)
            raise ValueError(f"Agenda.segments 中存在重复 id: {dupes}")
        ...
        if self.fallback_segment_id is not None and self.fallback_segment_id not in seg_ids:
            raise ValueError(
                f"Agenda.fallback_segment_id='{self.fallback_segment_id}' 不在 segments.id 中(可用: {sorted(seg_ids)})"
            )
...
def parse_agenda_toml(path: Path) -> Agenda:
    """从 TOML 文件加载并校验为 :class:`Agenda`。
    ...
    return Agenda.model_validate(data)
