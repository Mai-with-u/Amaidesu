...
模块注册了 4 个 MCP tool。Tool 名称即为客户端调用时的 identifier。

### `send_action`

对齐 MaiBot Plugin 的 `amaidesu_action` Tool，用于触发 Amaidesu 的结构化 action + emotion + text。

**参数表**

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `action_name` | string | 否 | `None` | 全限定 action 名,格式 `<handler>.<local_action>`(如 `warudo.wave`) |
| `action_parameters` | string(JSON) | 否 | `None` | 动作参数,如 `{"duration_ms": 1500}` |
| `emotion_name` | string | 否 | `None` | 情绪枚举名,12 值之一:`neutral` / `happy` / `sad` / `angry` / `surprised` / `shy` / `love` / `excited` / `confused` / `scared` / `thinking` / `relaxed` |
| `emotion_intensity` | float | 否 | `0.5` | 情绪强度,范围 [0.0, 1.0] |
| `text` | string | 否 | `None` | 附带回复文本(TTS 朗读) |

至少需要提供 `action_name` / `emotion_name` / `text` 之一。`action_name` 必须含 `.`(handler-qualified),否则 422 拒绝。
...
无参数。查询 Amaidesu Output 阶段所有 handler 暴露的全限定 action 列表（带 parameters 描述）。调用 `GET /api/v1/capabilities`，供 LLM 动态发现能力（避免硬编码列表与实际能力漂移）。
...
无参数。查询 Amaidesu Output 阶段所有合法 handler 名（供 LLM 构造 `action_name` 前缀）。调用 `GET /api/v1/handlers`。

**返回值示例**

```json
{"handlers": ["warudo", "vts", "vrchat", "tts", "subtitle"]}
```
...
| `amaidesu_query_capabilities` Tool | `@Tool` 装饰器，LLM 自动触发 | 支持，对应 `query_capabilities` | 替代旧的硬编码 action 列表 |
| `amaidesu_query_handlers` Tool | `@Tool` 装饰器，LLM 自动触发 | 支持，对应 `query_handler_names` | 供 LLM 构造 `action_name` 前缀 |
...
> ❌ `amaidesu_react` Tool 已删除(破坏性升级,新结构化 action + emotion + text 不再需要独立 "react" 工具)
> ❌ `send_react` MCP Tool 已删除(合并到 `send_action`,通过 `text` 参数表达台词)
...
4. `setup()` 内部创建 `FastMCP("amaidesu-mcp")` 实例、注册 4 个 tool(`send_action` / `query_capabilities` / `query_handler_names` / `get_status`)、并以 `asyncio.create_task` 方式启动 HTTP 监听
...
重启 Claude Desktop 后，工具面板会出现 `send_action`、`query_capabilities`、`query_handler_names`、`get_status` 四个工具，可直接在对话中调用。
...
| `send_action` 返回 422 Invalid action | action 名称未含 `.` 前缀或 handler 名不在合法列表 | 调用 `query_handler_names` 获取合法前缀,使用 `<handler>.<action>` 格式 |
| `send_action` 返回 422 Invalid emotion | emotion 名称不在 12 值枚举中 | 调用 `query_capabilities` 或使用枚举值:neutral/happy/sad/angry/surprised/shy/love/excited/confused/scared/thinking/relaxed |
