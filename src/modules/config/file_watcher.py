...
class FileWatcher:
    """基于 watchfiles 的文件监视器，支持多订阅、回调冷却、防抖等能力。

    该实现从 MaiBot 移植，保留全部行为契约以便 Amaidesu 接入。
    """
    ...
    def subscribe(
        ...
        *,
        ...
        Args:
            ...
            paths: 仅订阅这些路径（可包含目录）；None 表示订阅所有 self._paths。
            change_types: 仅订阅这些变更类型；None 表示订阅所有类型。
        """
        ...
        while self._running:
            try:
                if not self._ready_event.is_set():
                    # ready 事件需在第一次 await awatch 之前触发，避免调用方等到首个 timeout tick 才放行。
        ...
        for subscription in list(self._subscriptions.values()):
            ...
            try:
                await asyncio.wait_for(
                    self._invoke_callback(subscription.callback, matched_changes),
    ...
    def _match_changes(self, changes: Sequence[FileChange], subscription: FileWatchSubscription) -> list[FileChange]:
        """按订阅者的 paths/change_types 过滤本批次变更。"""
