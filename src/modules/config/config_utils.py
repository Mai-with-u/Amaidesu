            ...
            if os.path.isdir(component_item_path) and not item_name.startswith("__"):
                ...
                if template_exists and not config_exists:
                    try:
                        ...
                        logger.error(
                            f"在{component_type_name} '{item_name}' 中: 从模板复制配置文件失败: {e}", exc_info=True
                        )
...
def check_and_setup_pipeline_configs(pipeline_base_dir_name: str = "src/stages/input/pipelines") -> bool:
    """
...
def load_component_specific_config(
    ...
    Args:
        component_dir_path: 组件包的绝对路径 (例如 /path/to/src/stages/{domain}/providers/{provider})
...
def initialize_configurations(
    ...
    pipeline_dir_name: str = "src/stages/input/pipelines",
