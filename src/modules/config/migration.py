...
将根目录的旧 config.toml 迁移到 config/ 目录下的 7 个拆分文件。
...
文件映射：
    ...
    deciders（决策组件）→ agents.toml（enabled 列表 + 各 decider 子配置）
...
_DEAD_SECTIONS = {
    ...
    # 阶段组件段由 CrossFileMigration 合并到 tools/agents，
    # 此处仅记录 [providers.*] 等深废弃路径
    "providers",
}

# section 前缀 → 目标文件 映射（7 文件）
_SECTION_MAP: dict[str, str] = {
    ...
    "logging": "core.toml",
    "pipelines": "core.toml",
    # 核心段（若出现在待迁移的旧合并版 config.toml 中，归位 core.toml；
...
# 旧路径前缀 → 新 section 名（与 CrossFileMigration 机制配合）
