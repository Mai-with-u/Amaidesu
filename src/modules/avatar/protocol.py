...
三个皮套平台 Provider（``VTSProvider`` / ``WarudoProvider`` / ``VRChatProvider``）
同语义工具跨平台同名 + 同参数形状（``set_expression`` / ``list_preset_actions``
...
class AvatarProvider(Protocol):
    ...
    async def set_expression(self, emotion: str, intensity: float) -> object:
        ...
        同语义跨平台同名 + 同参数形状；返回 ``ToolExecutionResult`` 形态
        的调用结果（成功/失败 + 结构化载荷）。
        """
        ...
