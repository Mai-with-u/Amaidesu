...
（``"external"`` / ``"outline"`` / ``"schedule"`` / ``"cold"``）或 ``None``。

**不**负责生成发言内容 —— 话题文本由 ``Planner`` 在收到 ``proactive=True``
标志后从 ``room_state`` 快照的 ``topic_summary`` 字段（或在大纲触发时从
当前大纲环节任务描述）自行决定；本组件不持有 prompt 模板、不调用 LLM、
不预设话题 fallback 链。
...
- **触发优先级**：``external > outline > schedule > cold``（同一 tick 多
  触发条件同时满足时只取优先级最高的一个）。
...
- **话题缺失优雅降级**：``topic_required=True`` 且 ``topic_summary`` 为空
  ...
  ``outline`` 触发源不依赖 ``topic_summary``——大纲本身提供话题（由当前
  环节任务描述驱动），因此 outline 分支绕过 ``topic_required`` 约束。
- **触发源独立性**：``external`` / ``schedule`` / ``cold`` 三分支受
  ``topic_required`` 约束；``outline`` 分支不受其约束（见上）。

**大纲触发源（outline）的限流独立性**
------------------------------------
直播大纲是**内容推进**而非**救场**——主播应按环节节奏持续发言，不应被为
"防接龙/冷场救场"设计的低频限流卡死。具体规则：

- ``outline_pending=True``（环节切换即时信号）：仅受总开关约束，立即触发
  ``"outline"``，新环节开场白必须马上说，不等任何间隔。
- ``outline_ready=True``（环节内持续发言信号）：绕过公共 ``min_interval_ms``
  / ``max_per_hour`` / ``topic_required`` 三道前置，使用独立的
  ``outline_speech_interval_ms``（默认 3 秒防抖，对照 ``last_speech_ms``）
  控制发言节奏。观众弹幕回应仍会刷新 ``last_speech_ms``，自然推迟下一句
  大纲发言（刚回完观众就推话题很赶，这个联动保留）。
- 仅受总开关约束（``enabled=False`` 时不触发任何 outline）。
...
# 大纲环节内两次主动发言最小间隔（毫秒）。绕过通用 min_interval_ms（120s）
# 防接龙限流，使用防抖级别节奏（3 秒）保证大纲内容能持续推进；当前 LLM
# 生成耗时（~10s）远大于此值，实际开口节奏由 LLM 速度主导。
_DEFAULT_OUTLINE_SPEECH_INTERVAL_MS: int = 3_000
...
class ProactiveTrigger:
    ...
    def __init__(self, config: Any) -> None:
        ...
        # 大纲环节内持续发言间隔（毫秒）。仅 outline 触发源使用，绕过通用
        # min_interval_ms / max_per_hour / topic_required 限流。
        self._outline_speech_interval_ms: int = _cfg(
            config, "outline_speech_interval_ms", _DEFAULT_OUTLINE_SPEECH_INTERVAL_MS
        )
    ...
    def should_trigger(
        ...
        - ``outline`` **不**受 ``topic_required`` 约束（大纲本身提供话题，
          来自当前环节任务描述，不依赖 ``room_state.topic_summary``）。

        大纲触发源（outline）的限流独立性
        ---------------------------------
        直播大纲是**内容推进**而非**救场**——主播应按环节节奏持续发言，不应
        ...
        Args:
            ...
            external_pending: 是否有外部指令（如 WebUI API）待消费。
                **一次性**：调用方消费后应置 ``False``（通常下一 tick 即可，
                本组件不维护该标志）。
            outline_pending: 大纲环节切换即时信号（如 ``on_advance`` 回调
                触发）。``True`` 时仅受总开关约束，立即返回 ``"outline"``，
                不等任何间隔（即使 ``last_speech_ms`` 刚刚刷新）。
            outline_ready: 大纲是否已激活且具有当前环节。``True`` 时绕过
                ``min_interval_ms`` / ``max_per_hour`` / ``topic_required``
                三道公共前置，使用 ``outline_speech_interval_ms`` 控制节奏
                （对照 ``last_speech_ms``）。优先级低于 ``external``，高于
                ``schedule`` / ``cold``。

        Returns:
            ``"external"`` / ``"outline"`` / ``"schedule"`` / ``"cold"`` ——
        ...
        # outline_pending：环节切换即时信号——只受总开关约束，立即触发
        # （新环节开场白必须马上说，不等任何间隔）
        if outline_pending:
            return "outline"
        ...
        # outline：环节内持续发言——内容推进不是救场，绕过
        # min_interval/max_per_hour/topic_required，用自己的发言节奏
        # outline_speech_interval_ms（对照 last_speech_ms）
        if outline_ready:
            if last_speech_ms is not None and (now_ms - last_speech_ms) < self._outline_speech_interval_ms:
                return None
            return "outline"
