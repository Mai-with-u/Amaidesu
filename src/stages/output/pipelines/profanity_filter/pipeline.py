...
@pipeline("profanity_filter")
class ProfanityFilterOutputPipeline(Pipeline["Intent"]):
    """敏感词过滤管道。"""
    ...
    async def _process(self, item: "Intent") -> Optional["Intent"]:
        ...
        if item.speech:
            filtered_text, text_has_match = self._filter_text(item.speech)
            if text_has_match:
                has_match = True
                item.speech = filtered_text
    ...
    def _filter_text(self, text: str):
        ...
        if has_match:
            self._stats.filtered_words_count = getattr(self._stats, "filtered_words_count", 0) + len(matched_words)
