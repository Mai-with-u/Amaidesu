...
class InputPipelineManager:
    ...
    async def process(self, message: NormalizedMessage) -> Optional[NormalizedMessage]:
        ...
        在 Collector 产出消息之后、发布 INPUT_MESSAGE_RECEIVED 事件之前调用。
    ...
    async def load_pipelines(
        self,
        pipeline_base_dir: str = "src/stages/input/pipelines",
        ...
        for pipeline_name_snake, pipeline_config in input_pipelines.items():
            ...
            try:
                # 使用统一的模块导入路径格式
                module_import_path = f"src.stages.input.pipelines.{pipeline_name_snake}.pipeline"
