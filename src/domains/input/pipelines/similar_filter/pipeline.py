...
from src.domains.input.pipelines.manager import MessagePipelineBase, TextPipelineBase
...
class SimilarFilterTextPipeline(TextPipelineBase):
    ...
    async def _process(self, text: str, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        self.logger.debug(f"文本通过过滤并添加到缓存: {text[:30]!r}")
...
class SimilarFilterMessagePipeline(MessagePipelineBase):
    """
    相似文本过滤管道（MessagePipeline 版本）

    处理 NormalizedMessage，过滤短时间内内容高度相似的重复消息。

    与 SimilarFilterTextPipeline 的区别：
        - 输入是 NormalizedMessage 对象而非 (text, metadata) 元组
        - 从 NormalizedMessage.user_id 属性获取用户ID（自动处理 raw=None 情况）
        - group_id 从 NormalizedMessage.source 字段提取
        - 返回 NormalizedMessage 而非 text 字符串
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"SimilarFilterMessagePipeline 初始化: "
            f"相似度阈值={self._similarity_threshold}, "
            f"时间窗口={self._time_window}秒, "
            f"跨用户过滤={self._cross_user_filter}"
        )
