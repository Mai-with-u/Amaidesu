...
- 渲染表情参数和手势到 VRChat
- 软降级：python-osc 不可用时 Handler 仍能正常实例化
"""
...
from src.domains.output.providers.avatar.base import AvatarHandlerBase
...
class VRChatHandler(AvatarHandlerBase):
    ...
    def __init__(
        ...
        Args:
            config: Handler 配置（来自 [providers.output.vrchat]）
        ...
        if not self._osc_enabled:
            self.logger.warning(
                "python-osc 库不可用，VRChatHandler 将在禁用状态下运行。请安装 python-osc: `uv add python-osc`"
            )
        ...
        self.logger.info(f"VRChatHandler 初始化 - VRChat OSC: {self.vrc_host}:{self.vrc_out_port}")
