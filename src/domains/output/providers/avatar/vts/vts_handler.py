...
class VTSHandler(AvatarHandlerBase):
    ...
    def __init__(
        ...
        Args:
            config: Handler配置（来自[rendering.outputs.vts]）
    ...
    async def _adapt_intent(self, intent: "Intent") -> Dict[str, Any]:
        ...
        if intent.action:
            ...
            if action_type_str in self._action_hotkey_map:
                ...
                matched_hotkey = self._find_hotkey_by_name(hotkey_prefix, "")
                if matched_hotkey:
    ...
    async def _find_best_matching_hotkey_with_llm(self, text: str) -> Optional[str]:
        ...
        prompt = self.prompt_service.render("output/vts_hotkey", text=text, hotkey_list_str=hotkey_str)
