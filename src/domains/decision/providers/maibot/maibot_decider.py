"""
MaiBotDecider - MaiBot决策提供者

职责:
- 将 NormalizedMessage 转换为 Intent
- 通过 WebSocket 与 MaiBot 通信
- 自己解析 MaiBot 响应为 Intent（支持 LLM 和规则两种方式）
"""
...
from src.domains.decision.registry import decider
...
class MaiBotDecider:
    ...
    通过 WebSocket 与 MaiBot 通信，将 NormalizedMessage 转换为 Intent。

    职责:
    - 决策逻辑 (decide)
    - 直接使用 Router 管理 WebSocket 连接
    - 使用 RouterAdapter 发送消息到 MaiBot
    - 自己解析 MaiBot 响应为 Intent（支持 LLM 和规则两种方式）
    ...
    异步处理流程:
        1. decide() 被调用，发送消息到 MaiBot（fire-and-forget）
        2. MaiBot 响应到达时，_process_maibot_message 解析为 Intent（优先 LLM，失败时降级到规则）
        3. 通过 event_bus 发布 decision.intent 事件
    """

    class ConfigSchema(BaseProviderConfig):
        """MaiBot决策Decider配置Schema

        通过WebSocket与MaiBot通信进行决策。
        """
        ...
        # 结构化 Intent 配置
        structured_intent: bool = Field(
            default=True,
            description="使用结构化 Intent（MaiBot 插件直出 emotion/action），关闭后使用 LLM 解析",
        )
    ...
    def __init__(
        ...
        Args:
            config: 配置字典，包含:
                - host: MaiBot WebSocket 服务器主机
                - port: MaiBot WebSocket 服务器端口
                - platform: 平台标识符
            ...
            prompt_service: Prompt 服务实例（可选）
        """
        self.provider_name = "maibot"
    ...
    async def setup(self) -> None:
        ...
        self._router_adapter.register_message_handler(self._handle_maibot_message)
    ...
    def _normalized_to_message_base(self, normalized: "NormalizedMessage") -> Optional["MessageBase"]:
        ...
        将 NormalizedMessage 转换为 MaiBot 需要的 MessageBase 格式。
    ...
    async def decide(self, normalized_message: "NormalizedMessage") -> None:
        ...
        try:
            self.logger.info("发送到 MaiBot 的消息结构:")
    ...
    async def _process_maibot_message(self, message_data: Dict[str, Any]):
        ...
        1. 解析为 MessageBase
        2. 调用 _parse_intent_from_maibot_response() 构造 Intent
        3. 通过 event_bus 发布 decision.intent 事件
        ...
        self.logger.debug(f"收到 MaiBot 消息: message_id={message_id}, text={response_text}")
        self.logger.debug(f"完整消息: {message_data}")

        # 检查消息是否是 MaiBot 的响应（有 message_segment 且有内容）
        ...
        try:
            ...
            intent = self._parse_intent_from_maibot_response(message)
    ...
    async def _publish_intent(self, intent: Intent) -> None:
        ...
        provider_name = self.get_info().get("name", "maibot")
    ...
    def _extract_structured_intent(self, response: MessageBase) -> Optional[tuple[Optional[str], Optional[str]]]:
        """
        从 MaiBot 响应中提取结构化 emotion/action

        当 MaiBot 使用 amaidesu_plugin @Tool 时，会在 additional_config 中包含 emotion 和 action。

        Args:
            response: MaiBot 返回的 MessageBase

        Returns:
            (emotion, action) 元组，如果结构化数据找到则返回，否则返回 None
        """
        additional_config = response.message_info.additional_config or {}
        if "emotion" in additional_config or "action" in additional_config:
            emotion = additional_config.get("emotion")
            action = additional_config.get("action")
            return (emotion, action)
        return None

    def _parse_intent_from_maibot_response(self, response: MessageBase) -> Intent:
        """
        从 MaiBot 响应解析 Intent

        优先使用结构化数据（来自 MaiBot 插件），其次 LLM 解析，最后降级到规则解析。
        ...
        # 结构化 Intent 路径：检查配置和 structured data
        if self.typed_config.structured_intent:
            structured = self._extract_structured_intent(response)
            if structured:
                emotion, action = structured
                self.logger.info("使用结构化 Intent（零二次 LLM 调用）")
                return Intent(
                    emotion=emotion,
                    action=action,
                    speech=text,
            ...
            self.logger.warning("structured_intent=true 但未找到结构化数据，降级到 LLM 解析")
    ...
    def _extract_reply_to_id(self, message: MessageBase) -> Optional[str]:
        ...
        MaiBot 在消息间隔过长时会回复，回复消息的 Seg 中包含 type="reply" 的段，
        其 data 字段包含原始消息的 ID。

        Args:
            message: MaiBot 返回的 MessageBase
    ...
    async def _parse_intent_locally(self, normalized_message: "NormalizedMessage") -> Intent:
        """
        本地解析 Intent（不等待 MaiBot 响应）

        发送消息到 MaiBot 后，直接使用本地 LLM 或规则进行意图解析。
        ...
        if llm_service:
            try:
                ...
                return self._parse_intent_from_maibot_response(message)
            except Exception as e:
                self.logger.warning(f"LLM 解析失败，降级到规则解析: {e}")
