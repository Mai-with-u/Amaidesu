...
@decider("maicraft")
class MaicraftDecider:
    """
    Maicraft 弹幕互动游戏决策 Decider。
    ...
    class ConfigSchema(BaseProviderConfig):
        """Maicraft决策Decider配置Schema

        基于抽象工厂模式的弹幕互动游戏决策Decider。
        支持通过配置切换不同的动作实现系列（如 Log、MCP 等）。
        """
    ...
    def __init__(self, config: dict, event_bus: "EventBus"):
        ...
        # 解析配置
        try:
            self.parsed_config = MaicraftDeciderConfig(**config)
    ...
    def _create_default_intent(self, message: "NormalizedMessage") -> Intent:
        """
        创建默认 Intent（无动作）。
