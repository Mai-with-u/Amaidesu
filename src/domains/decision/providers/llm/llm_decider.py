...
class LLMDecider:
    ...
    def __init__(
        ...
        Args:
            ...
            llm_service: LLM Manager 实例（必填）
            prompt_service: Prompt Manager 实例（必填）
            config_service: ConfigService 实例（可选）
    ...
    async def setup(self) -> None:
        ...
        验证依赖是否正确注入。
        """
        self.logger.info("初始化 LLMDecider...")

        if self._llm_service is None:
            raise RuntimeError("LLM Manager 未注入！请确保在 setup 中正确配置。")

        self.logger.info(f"LLMDecider 初始化完成 (Client: {self.client_type})")
    ...
    async def decide(self, normalized_message: "NormalizedMessage") -> None:
        ...
        try:
            ...
            self.logger.info(f"LLMDecider 使用 LLM 解析意图: {normalized_message.text[:50]}...")
    ...
    def _clean_llm_json(self, raw_output: str) -> str:
        """
        清理 LLM 返回的 JSON 字符串（与 MaiBot 一致）
    ...
    async def cleanup(self) -> None:
        ...
        self.logger.info("LLMDecider已清理")
