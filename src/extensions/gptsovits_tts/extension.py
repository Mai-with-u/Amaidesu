"""
GPTSoVITS TTS插件Extension包装

将GPTSoVITSPlugin包装为Extension，保持原有功能不变。
使用GPTSoVITS引擎进行高质量语音合成，支持流式语音合成和口型同步。
"""
...
class CoreWrapper:
    ...
    GPTSoVITSPlugin需要WebSocket处理器注册和服务获取功能。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "gptsovits_tts_extension")
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        GPTSoVITSPlugin需要获取以下服务：
        - text_cleanup: 文本清理服务（可选）
        - subtitle_service: 字幕服务（可选）
        - vts_lip_sync: VTS口型同步服务（可选）
        - obs_control: OBS控制服务（可选）
        """
        return self._services.get(service_name)


class GPTSoVITSExtension(BaseExtension):
    """
    GPTSoVITS TTS插件Extension

    包装GPTSoVITSPlugin，将其作为Extension加载。
    使用GPTSoVITS引擎进行高质量语音合成，支持流式语音合成和口型同步。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.gptsovits_tts.plugin import GPTSoVITSPlugin

        # 创建插件实例
        self._plugin = GPTSoVITSPlugin(self._core_wrapper, config)
        self.logger.info("GPTSoVITSPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("GPTSoVITSExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("GPTSoVITSExtension清理中...")
        ...
        self.logger.info("GPTSoVITSExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="gptsovits_tts",
            version="1.0.0",
            description="GPTSoVITS TTS插件Extension包装（GPTSoVITS流式语音合成 + 口型同步）",
    ...
    def get_dependencies(self) -> List[str]:
        ...
        - obs_control: OBS控制服务（可选，由obs_control extension提供）
        """
        return []


# Extension入口点 - ExtensionManager会查找这个变量
extension_class = GPTSoVITSExtension
