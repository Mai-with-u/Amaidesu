"""
STT（语音转文字）插件Extension包装

将STTPlugin包装为Extension，保持原有功能不变。
支持VAD（语音活动检测）和讯飞API语音识别。
"""
...
class CoreWrapper:
    ...
    STTPlugin需要服务获取功能（stt_correction, prompt_context）。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "stt_extension")
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        STTPlugin需要获取以下服务：
        - stt_correction: STT文本修正服务（可选）
        - prompt_context: 上下文服务（可选）
        """
        return self._services.get(service_name)


class STTExtension(BaseExtension):
    """
    STT插件Extension

    包装STTPlugin，将其作为Extension加载。
    支持VAD（语音活动检测）和讯飞API语音识别。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.stt.plugin import STTPlugin

        # 创建插件实例
        self._plugin = STTPlugin(self._core_wrapper, config)
        self.logger.info("STTPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("STTExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("STTExtension清理中...")
        ...
        self.logger.info("STTExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="stt",
            version="1.0.0",
            description="STT插件Extension包装（VAD + 讯飞API语音识别）",
    ...
    def get_dependencies(self) -> List[str]:
        ...
        所有服务依赖都是可选的：
        - stt_correction: STT文本修正服务（可选，可能由llm_text_processor提供）
        - prompt_context: 上下文服务（可选）
        """
        return []
