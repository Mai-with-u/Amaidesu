...
class CoreWrapper:
    ...
    TTSPlugin需要服务注册和获取功能（text_cleanup, subtitle_service, vts_lip_sync）。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "tts_extension")
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        - vts_lip_sync_service: VTS口型同步服务（可选）
        """
        return self._services.get(service_name)


class TTSExtension(BaseExtension):
    """
    TTS插件Extension

    包装TTSPlugin，将其作为Extension加载。
    支持Edge TTS和Omni TTS，以及口型同步功能。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.tts.plugin import TTSPlugin

        # 创建插件实例
        self._plugin = TTSPlugin(self._core_wrapper, config)
        self.logger.info("TTSPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("TTSExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("TTSExtension清理中...")
        ...
        self.logger.info("TTSExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="tts",
            version="1.0.0",
            description="TTS插件Extension包装（Edge TTS + Omni TTS + 口型同步）",
    ...
    def get_dependencies(self) -> List[str]:
        ...
        所有服务依赖都是可选的：
        - text_cleanup: 文本清理服务（可选，由llm_text_processor提供，但该插件未实现）
