"""
RemoteStream插件Extension包装

将RemoteStreamPlugin包装为Extension，保持原有功能不变。
"""
...
class CoreWrapper:
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "remote_stream_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        RemoteStreamPlugin注册remote_stream服务
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        RemoteStreamPlugin不依赖其他服务
        """
        return self._services.get(service_name)


class RemoteStreamExtension(BaseExtension):
    """
    RemoteStream插件Extension

    包装RemoteStreamPlugin，将其作为Extension加载。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.remote_stream.plugin import RemoteStreamPlugin

        # 创建插件实例
        self._plugin = RemoteStreamPlugin(self._core_wrapper, config)
        self.logger.info("RemoteStreamPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("RemoteStreamExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("RemoteStreamExtension清理中...")
        ...
        self.logger.info("RemoteStreamExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="remote_stream",
            version="1.0.0",
            description="RemoteStream插件Extension包装",
...
extension_class = RemoteStreamExtension
