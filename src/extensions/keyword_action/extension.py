"""
Keyword Action插件Extension包装

将KeywordActionPlugin包装为Extension，保持原有功能不变。
支持关键词触发动作脚本，通过动态加载执行。
"""
...
class CoreWrapper:
    ...
    KeywordActionPlugin使用core执行动态加载的动作脚本。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "keyword_action_extension")
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        KeywordActionPlugin可能需要获取其他服务来执行动作脚本
        """
        return self._services.get(service_name)


class KeywordActionExtension(BaseExtension):
    """
    Keyword Action插件Extension

    包装KeywordActionPlugin，将其作为Extension加载。
    支持关键词触发动作脚本，通过动态加载执行。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.keyword_action.plugin import KeywordActionPlugin

        # 创建插件实例
        self._plugin = KeywordActionPlugin(self._core_wrapper, config)
        self.logger.info("KeywordActionPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("KeywordActionExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("KeywordActionExtension清理中...")
        ...
        self.logger.info("KeywordActionExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="keyword_action",
            version="1.0.0",
            description="Keyword Action插件Extension包装（关键词触发动作脚本）",
...
extension_class = KeywordActionExtension
