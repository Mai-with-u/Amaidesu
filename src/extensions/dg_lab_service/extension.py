"""
DG-Lab Service插件Extension包装

将DGLabServicePlugin包装为Extension，保持原有功能不变。
提供DG-LAB硬件控制服务。
"""
...
class CoreWrapper:
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "dg_lab_service_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        DGLabServicePlugin注册dg_lab_control服务供其他插件使用
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        DGLabServicePlugin不需要获取其他服务
        """
        return self._services.get(service_name)


class DGLabServiceExtension(BaseExtension):
    """
    DG-Lab Service插件Extension

    包装DGLabServicePlugin，将其作为Extension加载。
    提供DG-LAB硬件控制服务。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.dg_lab_service.plugin import DGLabServicePlugin

        # 创建插件实例
        self._plugin = DGLabServicePlugin(self._core_wrapper, config)
        self.logger.info("DGLabServicePlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("DGLabServiceExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("DGLabServiceExtension清理中...")
        ...
        self.logger.info("DGLabServiceExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="dg_lab_service",
            version="1.0.0",
            description="DG-Lab Service插件Extension包装（DG-LAB硬件控制服务）",
...
extension_class = DGLabServiceExtension
