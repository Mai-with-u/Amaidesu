"""
Sticker插件Extension包装

将StickerPlugin包装为Extension，保持原有功能不变。
"""
...
class CoreWrapper:
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "sticker_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        StickerPlugin不注册服务，但需要这个方法用于兼容
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        StickerPlugin需要从EventBus或ExtensionManager获取vts_control服务
        """
        # 在Extension.setup()中通过ExtensionManager获取服务
        return None


class StickerExtension(BaseExtension):
    """
    Sticker插件Extension

    包装StickerPlugin，将其作为Extension加载。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.sticker.plugin import StickerPlugin

        # 创建插件实例
        self._plugin = StickerPlugin(self._core_wrapper, config)
        self.logger.info("StickerPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("StickerExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("StickerExtension清理中...")
        ...
        self.logger.info("StickerExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="sticker",
            version="1.0.0",
            description="Sticker插件Extension包装",
    ...
    def get_dependencies(self) -> List[str]:
        ...
        这个Extension依赖vtube_studio Extension（提供vts_control服务）
        """
        return ["vtube_studio"]


# Extension入口点 - ExtensionManager会查找这个变量
extension_class = StickerExtension
