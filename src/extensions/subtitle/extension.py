"""
Subtitle插件Extension包装

将SubtitlePlugin包装为Extension，保持原有功能不变。
"""
...
class CoreWrapper:
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "subtitle_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        SubtitlePlugin注册subtitle_service服务
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        SubtitlePlugin不依赖其他服务
        """
        return self._services.get(service_name)


class SubtitleExtension(BaseExtension):
    """
    Subtitle插件Extension

    包装SubtitlePlugin，将其作为Extension加载。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 创建插件实例
        self._plugin = SubtitlePlugin(self._core_wrapper, config)
        self.logger.info("SubtitlePlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("SubtitleExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("SubtitleExtension清理中...")
        ...
        self.logger.info("SubtitleExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="subtitle",
            version="1.0.0",
            description="Subtitle插件Extension包装",
...
extension_class = SubtitleExtension
