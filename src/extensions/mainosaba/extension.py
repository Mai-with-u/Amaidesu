"""
Mainosaba插件Extension包装

将MainosabaPlugin包装为Extension，保持原有功能不变。
《魔法少女的魔女裁判》游戏助手插件，通过截图识别游戏台词。
"""
...
class CoreWrapper:
    ...
    MainosabaPlugin需要WebSocket处理器注册和服务获取功能。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "mainosaba_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        MainosabaPlugin注册mainosaba_command_handler服务供其他插件使用
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        MainosabaPlugin可能需要获取其他服务
        """
        return self._services.get(service_name)


class MainosabaExtension(BaseExtension):
    """
    Mainosaba插件Extension

    包装MainosabaPlugin，将其作为Extension加载。
    《魔法少女的魔女裁判》游戏助手插件，通过截图识别游戏台词。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 创建插件实例
        self._plugin = MainosabaPlugin(self._core_wrapper, config)
        self.logger.info("MainosabaPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("MainosabaExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("MainosabaExtension清理中...")
        ...
        self.logger.info("MainosabaExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="mainosaba",
            version="1.0.0",
            description="Mainosaba插件Extension包装（魔法少女游戏助手，截图识别游戏台词）",
...
extension_class = MainosabaExtension
