"""
Screen Monitor插件Extension包装

将ScreenMonitorPlugin包装为Extension，保持原有功能不变。
屏幕监控插件，AI分析屏幕内容。
"""
...
class CoreWrapper:
    ...
    ScreenMonitorPlugin需要WebSocket处理器注册和服务获取功能。
    """
    ...
    async def send_to_maicore(self, message: Any) -> None:
        ...
        """
        await self.event_bus.emit("input.raw_data", message, "screen_monitor_extension")
    ...
    def register_service(self, service_name: str, service: Any) -> None:
        ...
        ScreenMonitorPlugin注册prompt_context服务供其他插件使用
        """
    ...
    def get_service(self, service_name: str) -> Any:
        ...
        ScreenMonitorPlugin可能需要获取remote_stream服务
        """
        return self._services.get(service_name)


class ScreenMonitorExtension(BaseExtension):
    """
    Screen Monitor插件Extension

    包装ScreenMonitorPlugin，将其作为Extension加载。
    屏幕监控插件，AI分析屏幕内容。
    """
    ...
    async def setup(self, event_bus: Any, config: Dict[str, Any]) -> List[Any]:
        ...
        # 延迟导入插件，避免循环依赖
        from src.plugins.screen_monitor.plugin import ScreenMonitorPlugin

        # 创建插件实例
        self._plugin = ScreenMonitorPlugin(self._core_wrapper, config)
        self.logger.info("ScreenMonitorPlugin实例创建完成")

        # 调用插件的setup
        await self._plugin.setup()
        self.logger.info("ScreenMonitorExtension设置完成")
    ...
    async def cleanup(self) -> None:
        ...
        清理Extension
        """
        self.logger.info("ScreenMonitorExtension清理中...")
        ...
        self.logger.info("ScreenMonitorExtension清理完成")
    ...
    def get_info(self) -> ExtensionInfo:
        ...
        return ExtensionInfo(
            name="screen_monitor",
            version="1.0.0",
            description="Screen Monitor插件Extension包装（AI分析屏幕内容）",
...
extension_class = ScreenMonitorExtension
