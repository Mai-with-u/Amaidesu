...
class LLMExecutor:
    ...
    async def _execute_expression(self, arguments: Dict[str, Any], adapter_name: Optional[str]) -> Dict[str, Any]:
        """执行表情设置（语义动作）
        ...
        if not expression:
            return {"success": False, "error": "缺少必需参数: expression"}

        # 验证强度范围
        if not 0.0 <= intensity <= 1.0:
            return {"success": False, "error": f"intensity 必须在 0.0-1.0 范围内，当前值: {intensity}"}

        # 检查语义动作是否存在
        if not self.manager.semantic_mapper.has_action(expression):
            self.logger.warning(f"未知的语义动作: {expression}")
            return {"success": False, "error": f"未知的语义动作: {expression}"}

        # 获取目标适配器
        adapter = self._get_target_adapter(adapter_name)
        if not adapter:
            return {"success": False, "error": f"未找到适配器: {adapter_name or 'active'}"}

        # 获取语义动作映射
        mapping = self.manager.semantic_mapper.get_mapping(expression, adapter.adapter_name)

        if not mapping:
            return {"success": False, "error": f"语义动作 '{expression}' 没有针对平台 '{adapter.adapter_name}' 的映射"}

        # 应用强度系数
        adjusted_mapping = self.manager.semantic_mapper.apply_intensity(mapping, intensity)
    ...
    async def _execute_parameters(self, arguments: Dict[str, Any], adapter_name: Optional[str]) -> Dict[str, Any]:
        """执行参数设置（直接参数控制）
        ...
        if not parameters:
            return {"success": False, "error": "没有要设置的参数"}
    ...
    async def _execute_action(self, arguments: Dict[str, Any], adapter_name: Optional[str]) -> Dict[str, Any]:
        """执行动作触发
        ...
        # 找到设置为 true 的动作
        triggered_actions = [action_name for action_name, value in arguments.items() if value is True]

        if not triggered_actions:
            return {"success": False, "error": "没有触发的动作"}
