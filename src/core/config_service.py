...
class ConfigService:
    ...
    def get_provider_config_with_defaults(
        ...
        if provider_layer == "input":
            provider_dir = os.path.join(self.base_dir, "src", "layers", "input", "providers", provider_name)
            config_section = "providers.input"
        elif provider_layer == "output":
            provider_dir = os.path.join(self.base_dir, "src", "layers", "rendering", "providers", provider_name)
            config_section = "providers.output"
        elif provider_layer == "decision":
            provider_dir = os.path.join(self.base_dir, "src", "layers", "decision", "providers", provider_name)
    ...
    def _generate_config_from_schema(
        ...
        try:
            # 获取Schema的默认值
            schema_instance = schema_class()
            config_data = schema_instance.model_dump(exclude_unset=True)
