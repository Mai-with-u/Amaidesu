...
使用示例:
    from src.core.config.generator import generate_toml, ensure_provider_config
...
def ensure_provider_config(
    ...
    config_filename: str = "config-defaults.toml",
    ...
    if provider_layer == "input":
        provider_dir = os.path.join(base_dir, "src", "layers", "input", "providers", provider_name)
    elif provider_layer == "output":
        provider_dir = os.path.join(base_dir, "src", "layers", "rendering", "providers", provider_name)
    elif provider_layer == "decision":
        provider_dir = os.path.join(base_dir, "src", "layers", "decision", "providers", provider_name)
    else:
...
def _detect_project_root() -> str:
    ...
    # 如果找不到，假设当前目录是 src/core/config，返回 src 的父目录
