...
from .providers.bili_official_provider import BiliDanmakuOfficialInputProvider
...
class BiliDanmakuOfficialPlugin:
    ...
    - 不继承BasePlugin
    - 实现Plugin协议
    - 返回Provider列表（BiliDanmakuOfficialInputProvider）
    """
    ...
    async def setup(self, event_bus: "EventBus", config: Dict[str, Any]) -> List[Any]:
        ...
        Returns:
            Provider列表（包含BiliDanmakuOfficialInputProvider）
        """
        self.event_bus = event_bus

        if not self.enabled:
            self.logger.warning("BiliDanmakuOfficialPlugin 未启用，不创建Provider。")
            return []
        ...
        try:
            ...
            self.logger.error(f"创建 BiliDanmakuOfficialInputProvider 失败: {e}", exc_info=True)
            return []
