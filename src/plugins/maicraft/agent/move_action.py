...
from src.plugins.maicraft.agent.nearby_block import NearbyBlockManager
...
from src.plugins.maicraft.agent.utils import parse_json
...
class MoveAction:
    def __init__(self,config:MaicraftConfig):
        ...
        model_config = ModelConfig(
            model_name=self.config.llm_fast.model,
            api_key=self.config.llm_fast.api_key,
            base_url=self.config.llm_fast.base_url,
            max_tokens=self.config.llm_fast.max_tokens,
            temperature=self.config.llm_fast.temperature
        )
