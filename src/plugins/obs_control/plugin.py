...
class ObsControlPlugin(BasePlugin):
    def __init__(self, core: AmaidesuCore, plugin_config: Dict[str, Any]):
        ...
        self.logger.info(f"逐字显示功能：{'启用' if self.typewriter_enabled else '禁用'}，速度：{self.typewriter_speed}s/字符")
    ...
    async def handle_message(self, message: MessageBase):
        ...
        try:
            ...
            if self.typewriter_enabled:
                ...
                self.logger.info(f"已将消息发送至 OBS：{message_content[:50]}{'...' if len(message_content) > 50 else ''}")
