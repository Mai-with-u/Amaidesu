...
项目采用 **配置驱动的按需加载机制**，Provider 的加载完全由配置文件决定，未启用的 Provider 不会被导入或初始化。

#### 核心特性

1. **配置驱动**：Provider 的加载完全由配置文件驱动，只有配置中启用的 Provider 才会被加载
2. **按需加载**：只加载配置中启用的 Provider，不加载未使用的 Provider
3. **依赖隔离**：未使用的 Provider 依赖缺失不影响启动（如 STT Provider 需要 PyAudio，但未启用时不需要安装）

#### 性能优势

| 指标 | 全量加载 | 按需加载 | 提升 |
|------|----------|----------|------|
| **启动时间** | 导入所有 Provider 模块 | 只导入配置中的 Provider | ~50-70% |
| **内存占用** | 加载所有 Provider 类 | 只加载启用的 Provider | ~40-60% |
| **依赖要求** | 必须安装所有依赖 | 只需安装启用 Provider 的依赖 | 灵活部署 |

#### 注册时机

Provider 的注册在应用启动时自动完成，流程如下：

1. **配置加载**：从 `config.toml` 读取 Provider 配置
2. **模块导入**：根据配置动态导入 Provider 模块（`src/domains/*/providers/*`）
3. **自动注册**：Provider 模块的 `__init__.py` 执行时自动注册到 ProviderRegistry
4. **实例化**：Manager 根据配置创建启用的 Provider 实例
...
**说明**：
- `enabled_inputs` / `enabled_outputs`：配置需要启用的 Provider 列表
- `available_providers`：配置所有可用的 DecisionProvider（从其中选择 `active_provider`）
- 只有配置中的 Provider 会被导入和初始化

#### 自定义插件支持

支持通过 `module_path` 配置加载自定义 Provider（插件机制）：

```toml
[providers.output.outputs.my_custom]
type = "custom"
...
class MyCustomProvider(OutputProvider):
    """自定义输出 Provider"""
...
ProviderRegistry.register_output("custom", MyCustomProvider, source="plugin:my_custom")
```

#### 实现原理

Provider 的按需加载通过以下机制实现：

1. **延迟导入**：Provider 模块不会在顶层被导入（避免 `import src.domains.input.providers.*`）
2. **动态加载**：Manager 在启动时根据配置动态导入 Provider 模块
3. **自动注册**：Provider 模块的 `__init__.py` 执行时自动注册到 ProviderRegistry
4. **配置验证**：加载前验证配置完整性，避免运行时错误

#### 对比：全量加载 vs 按需加载

**全量加载（旧方式）**：
```python
# 导入所有 Provider（未使用的也会被加载）
...
from src.domains.input.providers.youtube_chat import YouTubeChatInputProvider
# ... 所有 Provider

# 所有 Provider 的依赖必须安装，否则启动失败
```

**按需加载（新方式）**：
```python
# 只导入配置中的 Provider
enabled_providers = config["providers"]["input"]["enabled_inputs"]
for provider_name in enabled_providers:
    module_path = f"src.domains.input.providers.{provider_name}"
    importlib.import_module(module_path)  # 动态导入

# 未使用的 Provider 依赖缺失不影响启动
```
