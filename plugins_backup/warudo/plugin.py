...
class WarudoPlugin(BasePlugin):
    ...
    def __init__(self, core: AmaidesuCore, plugin_config: Dict[str, Any]):
        ...
        if self.subtitle_enabled:
            ...
            self.subtitle_manager = ReplyGenerationManager(
                port=self.subtitle_port, show_status=self.subtitle_show_status, logger=self.logger
            )
        else:
            self.subtitle_manager = None

        # watching消息翻译表
        self.watching_translation = {"lens": "相机", "danmu": "弹幕", "wandering": None}
    ...
    async def setup(self):
        ...
        if not self.lip_sync_enabled:
            self.logger.error(
                "SETUP FAILED for WarudoPlugin: Lip-sync is disabled in config or dependencies (librosa, scipy) are missing."
            )
            return
    ...
    async def _connect_websocket(self):
        ...
        while True:
            try:
                ...
                self.logger.error(
                    f"An unexpected error occurred with WebSocket connection: {e}. Reconnecting in 5 seconds..."
                )
            finally:
                self._is_connected = False
                # 断开时不停止任务，让服务继续运行
    ...
    async def _update_lip_sync_parameters(self, analysis_result: Dict[str, float]):
        ...
        for vowel in self.vowel_formants.keys():
            ...
            self.current_vowel_values[vowel] = self.current_vowel_values[vowel] * smoothing + analysis_result.get(
                key, 0.0
            ) * (1 - smoothing)
        ...
        # 映射元音到LipSyncState
        vowel_mapping = {"A": "VowelA", "I": "VowelI", "U": "VowelU", "E": "VowelE", "O": "VowelO"}
