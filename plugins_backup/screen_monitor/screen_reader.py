...
class ScreenReader:
    ...
    def __init__(
        ...
        try:
            self.openai_client = AsyncOpenAI(api_key=api_key, base_url=base_url, timeout=timeout_seconds)
    ...
    async def _analyze_screen_image(
        self, image_base64: str, images_count: int, metadata: Dict[str, Any]
...
"""
    ...
    def _parse_llm_response(self, response: str, metadata: Dict[str, Any]) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            # 创建分析结果
            result = AnalysisResult(new_current_context=response)
            ...
            # 返回备用结果
            return AnalysisResult(new_current_context="屏幕内容已更新 (AI分析异常)")
    ...
    async def _update_contexts(self, result: AnalysisResult):
        ...
        if self.current_context != "当前屏幕内容尚未获取":
            history_item = ContextHistoryItem(
                timestamp=time.time(), context=self.current_context, trigger_reason="screen_change"
            )
    ...
    def get_context_history(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        return [
            {"timestamp": item.timestamp, "context": item.context, "trigger_reason": item.trigger_reason}
            for item in history
        ]
    ...
    def update_main_context_manually(self, new_main_context: str):
        ...
        # 添加到历史记录
        history_item = ContextHistoryItem(
            timestamp=time.time(), context=f"主上下文手动更新: {new_main_context}", trigger_reason="manual_update"
        )
        self.context_history.append(history_item)
...
async def example_usage():
    ...
    async def on_context_update(data):
        ...
        print(
            f"  📦 缓存图像: {stats['current_cache_size']}/{stats['cached_images_count']}, 拼接分析: {stats['stitched_analyses_count']}"
        )
