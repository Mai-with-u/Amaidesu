...
class ActionRegistry:
    ...
    示例:
        "chat" 命令 -> ActionType.CHAT
        "say" 命令 -> ActionType.CHAT
        "attack" 命令 -> ActionType.ATTACK
    """

    def __init__(self):
        self.logger = get_logger("ActionRegistry")
        # 命令名称到动作类型的映射
        self._command_to_action_type: Dict[str, ActionType] = {}

    def register_command(self, command_name: str, action_type: ActionType):
        """
        注册命令到动作类型的映射。
        ...
        if not isinstance(action_type, ActionType):
            raise ValueError(f"action_type 必须是 ActionType 枚举: {type(action_type)}")
    ...
    def get_action_type(self, command_name: str) -> Optional[ActionType]:
        """
        根据命令名称获取对应的动作类型。
    ...
    def get_commands_for_action_type(self, action_type: ActionType) -> list[str]:
        """
        获取映射到指定动作类型的所有命令。
    ...
    def load_from_config(self, command_mappings: Dict[str, str]):
        """
        从配置字典加载命令映射。
