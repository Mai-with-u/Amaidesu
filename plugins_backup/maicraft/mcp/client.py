...
class MCPClient:
    ...
    def _check_connection_health_immediately(self) -> None:
        ...
        # 如果连续失败次数过多，触发重连
        if (
            self._connection_health.consecutive_failures >= 3
            and self._reconnection_config.enabled
            and self._connection_state == ConnectionState.CONNECTED
        ):
            self.logger.warning(
                f"[MCP] 检测到连接问题，连续失败 {self._connection_health.consecutive_failures} 次，启动重连"
            )
    ...
    def _diagnose_connection_error(self, error: Exception) -> ConnectionErrorType:
        ...
        if any(
            keyword in error_str
            for keyword in ["authentication failed", "unauthorized", "forbidden", "invalid credentials"]
        ):
        ...
        if any(
            keyword in error_str
            for keyword in ["invalid config", "configuration error", "missing config", "config file not found"]
        ):
    ...
    def _get_error_message(self, error_type: ConnectionErrorType, original_error: str) -> str:
        ...
        messages = {
            ...
            ConnectionErrorType.UNKNOWN_ERROR: (f"未知连接错误：{original_error}\n请检查服务器和客户端的配置和状态。"),
        }
        ...
        try:
            ...
            if error_type in [
                ...
                if self._connection_health.consecutive_failures >= 3:
                    should_reconnect = True
                    self.logger.warning(
                        f"[MCP] 连续失败次数过多 ({self._connection_health.consecutive_failures})，准备重连"
                    )
                else:
            ...
            return CallToolResult(
                ...
                structured_content={"error_type": error_type.value, "tool_name": tool_name, "original_error": str(e)},
                is_error=True,
                data=None,
            )
    ...
    async def _perform_health_check(self) -> None:
        ...
        try:
            # 尝试获取工具列表作为健康检查
            tools = await asyncio.wait_for(self.list_available_tools(), timeout=10.0)
            ...
            if error_type in [
                ...
                should_reconnect = True
                self.logger.warning(
                    f"[MCP] 健康检查连续失败 {self._connection_health.consecutive_failures} 次，准备重连"
                )
    ...
    async def _reconnection_loop(self) -> None:
        ...
        while (
            ...
            and attempt < self._reconnection_config.max_attempts
            and self._connection_state != ConnectionState.CONNECTED
        ):
            if self._shutdown_event.is_set():
                break
