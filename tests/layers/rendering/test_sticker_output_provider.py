...
from src.layers.rendering.providers.sticker import StickerOutputProvider
...
def sticker_config():
    ...
    return {
        "sticker_size": 0.5,
        "sticker_rotation": 90,
        "sticker_position_x": 100,
        "sticker_position_y": 200,
        "image_width": 128,
        "image_height": 128,
        "cool_down_seconds": 3,
...
class TestStickerOutputProvider:
    ...
    def test_init_with_custom_config(self, sticker_config):
        ...
        assert provider.sticker_rotation == 90
        assert provider.sticker_position_x == 100
        assert provider.sticker_position_y == 200
    ...
    async def test_render_internal_success(self, sticker_config, mock_event_bus, sample_image_base64):
        ...
        # 验证send_to_vts被调用
        provider._send_to_vts.assert_called_once()
        # 验证冷却时间已更新
    ...
    async def test_send_to_vts(self, sticker_config, sample_image_base64):
        ...
        # Mock logger
        provider.logger.debug = MagicMock()

        # 发送到VTS（目前只是记录日志）
        await provider._send_to_vts(sample_image_base64)

        # 验证日志被调用（我们无法验证具体内容，因为它是mock）
