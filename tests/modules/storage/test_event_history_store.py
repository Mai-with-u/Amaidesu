...
async def store(tmp_path: Path) -> AsyncGenerator[SQLiteStore, None]:
    s = SQLiteStore(tmp_path / "event-history.db")
    await s.initialize()
    yield s
    await s.close()
...
async def _wait_for_row_count(store: SQLiteStore, expected: int, timeout: float = 3.0) -> None:
...
@pytest.mark.asyncio
async def test_insert_and_roundtrip_by_date(store: SQLiteStore) -> None:
    ...
    dates = await store.list_event_dates("room.message.danmaku")
    assert dates == ["2026-09-01"]

    rows = await store.get_day_events("2026-09-01", event_name="room.message.danmaku")
    assert len(rows) == 1
...
@pytest.mark.asyncio
async def test_list_event_dates_filters_by_name_and_sorted(store: SQLiteStore) -> None:
    """日期清单按事件名过滤、时间正序。"""
    ...
    assert await store.list_event_dates("room.message.danmaku") == ["2026-09-01", "2026-09-02"]
    assert await store.list_event_dates("core.error") == ["2026-09-01"]
    assert await store.list_event_dates("game.milestone") == []


@pytest.mark.asyncio
async def test_get_day_events_without_name_returns_all(store: SQLiteStore) -> None:
    ...
    rows = await store.get_day_events("2026-09-01")
    assert {row["event_name"] for row in rows} == {"room.message.danmaku", "core.startup"}


@pytest.mark.asyncio
async def test_service_record_persists_with_event_name_fallback(store: SQLiteStore) -> None:
    """service.record 异步写库；event_name 缺省时退回 type。"""
    service = EventHistoryService(max_events=10, persist=True, sqlite_store=store)
    service.record(
        EventRecord(
            id="evt-1",
            type="room.message",
...
async def test_service_record_without_store_keeps_buffer_only() -> None:
    ...
    service = EventHistoryService(max_events=10, persist=True, sqlite_store=None)
...
async def test_backfill_today_from_store(tmp_path: Path) -> None:
    ...
    try:
        service = EventHistoryService(max_events=100, persist=True, sqlite_store=reader)
...
@pytest.mark.asyncio
async def test_backfill_skips_non_today_rows(store: SQLiteStore) -> None:
    """回灌只取当日：非当日记录不进缓冲。"""
    await store.insert_event(
        record_id="old-row",
        event_name="room.message.danmaku",
    ...
    service = EventHistoryService(max_events=100, persist=True, sqlite_store=store)
    count = await service.backfill_today_from_store()
    assert count == 0


@pytest.mark.asyncio
async def test_uuid_record_id_roundtrip(store: SQLiteStore) -> None:
    ...
    rows = await store.get_day_events("2026-09-01", event_name="core.startup")
