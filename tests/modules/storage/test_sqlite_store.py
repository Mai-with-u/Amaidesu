...
@pytest.mark.asyncio
async def test_all_expected_tables_created(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_schema_migration_recorded(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_assert_schema_ready_passes(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_simulated_column_present_on_live_chat(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_simulated_column_present_on_gifts(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_simulated_column_present_on_super_chats(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_simulated_exclusion_query(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_is_healthy(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_message_type_values_live_chat(store: SQLiteDatabase) -> None:
...
# =============================================================================
# ConnectionManager（被 SQLiteDatabase 间接覆盖；此为直接单元测试）
# =============================================================================
...
@pytest.mark.asyncio
async def test_list_recent_live_chat_returns_chronological(store: SQLiteDatabase) -> None:
    ...
    rows = await store.chat.list_recent_live_chat(live_session_id=1, limit=3)
...
@pytest.mark.asyncio
async def test_list_recent_live_chat_before_window(store: SQLiteDatabase) -> None:
    ...
    rows = await store.chat.list_recent_live_chat(live_session_id=1, limit=10, before_timestamp_ms=4000)
    timestamps = [int(r["timestamp_ms"]) for r in rows]
...
@pytest.mark.asyncio
async def test_list_recent_live_chat_session_isolation(store: SQLiteDatabase) -> None:
    ...
    rows1 = await store.chat.list_recent_live_chat(live_session_id=1)
    rows2 = await store.chat.list_recent_live_chat(live_session_id=2)
    assert len(rows1) == 1
    assert len(rows2) == 1
...
@pytest.mark.asyncio
async def test_list_messages_by_user_filters_and_window(store: SQLiteDatabase) -> None:
    ...
    rows = await store.chat.list_messages_by_user(user_id="u1", since_timestamp_ms=0)
    timestamps = [int(r["timestamp_ms"]) for r in rows]
    assert timestamps == [1000, 3000, 4000], f"u1 全量错误，实得 {timestamps}"

    # u1 + 窗口 since=2000
    rows2 = await store.chat.list_messages_by_user(user_id="u1", since_timestamp_ms=2000)
    timestamps2 = [int(r["timestamp_ms"]) for r in rows2]
    assert timestamps2 == [3000, 4000], f"u1 窗口错误，实得 {timestamps2}"

    # u2 全量
    rows_u2 = await store.chat.list_messages_by_user(user_id="u2", since_timestamp_ms=0)
    assert [int(r["timestamp_ms"]) for r in rows_u2] == [2000, 5000]

    # 不存在的用户
    rows_none = await store.chat.list_messages_by_user(user_id="nobody", since_timestamp_ms=0)
    assert rows_none == []


@pytest.mark.asyncio
async def test_upsert_viewer_message_count(store: SQLiteDatabase) -> None:
...
@pytest.mark.asyncio
async def test_upsert_viewer_gift_count(store: SQLiteDatabase) -> None:
...
async def test_list_viewer_stats_order_by_whitelist(store: SQLiteDatabase) -> None:
    ...
    with pytest.raises(ValueError):
        await store.viewers.list_viewer_stats(order_by="user_name")  # noqa: S106 白名单外
    with pytest.raises(ValueError):
        await store.viewers.list_viewer_stats(order_by="1; DROP TABLE viewers--")  # noqa: S106 注入尝试
    with pytest.raises(ValueError):
        await store.viewers.list_viewer_stats(order_by="")  # noqa: S106 空字符串
