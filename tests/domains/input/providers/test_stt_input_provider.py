...
@pytest.mark.skip(reason="需要外部音频设备")
class TestSTTInputProvider:
    ...
    def test_init_with_custom_config(self, mock_deps, stt_config, mock_provider_context):
        """测试自定义配置初始化"""
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_sample_rate_validation(self, mock_deps, stt_config, mock_provider_context):
        """测试采样率验证"""
        from src.domains.input.providers.stt import STTInputProvider

        config = stt_config.copy()
        config["audio"]["sample_rate"] = 22050
        provider = STTInputProvider(config, context=mock_provider_context)

        assert provider.sample_rate == 16000

    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_block_size_calculation(self, mock_deps, stt_config, mock_provider_context):
        """测试块大小计算"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_block_size_for_8khz(self, mock_deps, stt_config, mock_provider_context):
        """测试 8kHz 的块大小"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_iflytek_auth_url_building(self, mock_deps, stt_config, mock_provider_context):
        """测试讯飞认证 URL 构建"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_iflytek_common_params(self, mock_deps, stt_config, mock_provider_context):
        """测试讯飞公共参数构建"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_iflytek_business_params(self, mock_deps, stt_config, mock_provider_context):
        """测试讯飞业务参数构建"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_iflytek_data_params(self, mock_deps, stt_config, mock_provider_context):
        """测试讯飞数据参数构建"""
        from src.domains.input.providers.stt import STTInputProvider
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    def test_iflytek_frame_building(self, mock_deps, stt_config, mock_provider_context):
        """测试讯飞帧构建"""
        import json
    ...
    @patch("src.domains.input.providers.stt.stt_input_provider.STTInputProvider._check_dependencies")
    @pytest.mark.asyncio
    async def test_cleanup(self, mock_deps, stt_config, mock_provider_context):
        """测试资源清理"""
        from src.domains.input.providers.stt import STTInputProvider
