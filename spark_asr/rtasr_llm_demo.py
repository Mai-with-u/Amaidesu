                ...
                if isinstance(msg, str):
                    try:
                        ...
                        if msg_json.get('msg_type') == 'result' and msg_json.get('res_type') == 'asr':
                            ...
                            # 尾包（ls=True）延迟：相对结束发送时间
                            try:
                                ls_flag = bool(msg_json.get('ls')) or bool(msg_json.get('data', {}).get('ls'))
                            except Exception:
                                ls_flag = False
                            if ls_flag and self.end_sent_ms is not None:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("【提示】正在实时采集并发送音频，按 Ctrl+C 结束录音并发送结束标记...")
        ...
        # 发送完成后，额外等待3秒接收尾部结果
        tail_wait = 3
        print(f"【等待结果】已结束发送，额外等待{tail_wait}秒接收尾包...")
        for _ in range(tail_wait):
