...
class SparkRTASRConfig(BaseModel):
    """Spark RTASR配置模型"""
    app_id: str = Field(default="", description="APP ID")
    access_key_id: str = Field(default="", description="API密钥ID")
    access_key_secret: str = Field(default="", description="API密钥")


class MaicraftConfig(BaseModel):
    """仅包含与 LLM/VLM 调用相关的配置"""
    ...
    spark_rtasr: SparkRTASRConfig = Field(default_factory=SparkRTASRConfig, description="Spark RTASR配置")
    
    class Config:
        extra = "ignore"  # 忽略非LLM相关字段，兼容旧的 config-template.toml
